...
- [ ] **Provide an independent control-loop watchdog with a hardware-safe steering inhibit** (2026-09-26). The current task watchdog checks idle tasks at 5 seconds, does not register control, and has panic disabled (`sdkconfig.esp32-s3-devkitc-1`). A firmware safety function cannot act when its own loop stalls. Reset alone is unsafe until steering PWM/direction have a physical safe state (the existing floating-pin repair below). Verify shutdown and motor outputs during an injected loop stall after the hardware inhibit exists.

- [⏸ requires confirmed steering motor isolation before flashing] **Validate centralized sensor safety on the kart** (2026-09-26). The implementation provides local drive permission, latched faults, healthy explicit reset, 20 Hz safety status and bench steering without propulsion. Native tests pass (73 total, including 11 safety-policy tests), and ESP32-S3/classic firmware links. After isolation, flash and check unplug/reconnect sensor, pressure loss, communications/state loss, startup inhibit, reset rejection/acceptance, manual/bench mode changes and physical shutdown braking. Measure control-loop rate with the added safety report. No hardware behavior is validated yet.
